 ...
 */
public class JawrCommandLineRunner extends
    JawrAbstractCommandLineRunner<Compiler, CompilerOptions> {
   ...
   */
  protected JawrCommandLineRunner(String[] args) {
    super();
    initConfigFromFlags(args, System.err);
  }

  protected JawrCommandLineRunner(String[] args, PrintStream out, PrintStream err) {
    super(out, err);
  ...
  public static List<JSSourceFile> getDefaultExterns() throws IOException {
    InputStream input = JawrCommandLineRunner.class.getResourceAsStream(
        "/externs.zip");
   ...
   */
  public static void main(String[] args) {
    JawrCommandLineRunner runner = new JawrCommandLineRunner(args);
    if (runner.shouldRunCompiler()) {
      runner.run();
    } else {
      System.exit(-1);
